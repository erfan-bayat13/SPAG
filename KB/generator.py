...
class HateAssessmentSystem:
    ...
    def process_query(self, user_query: str, search_method: str = "hybrid", 
        ...
        print(f"Retrieved {len(retrieved_content)} items from KB.")
    ...
    def _retrieve_content(self, query: str, search_method: str, limit: int) -> List[Dict]:
        ...
        if search_method == "syntax":
            ...
            for result in syntax_results + semantic_results:
        ...
        for item in raw_results:
            ...
            processed_item = {
                ...
                # Keep any additional metadata that might be useful
                "similarity_score": item.get("similarity_score", 0),
                "layer": item.get("layer", 1),  # Default to layer 1 if not specified
    ...
    def _calculate_hate_score(self, query: str, retrieved_content: List[Dict]) -> Tuple[float, Dict]:
        ...
        for item in retrieved_content:
            ...
            # Since similarity_score and relevance_score might not be available or reliable,
            # we'll need to derive it from another source or use a default value
            
            # Option 1: If we have layer information, we could use that as a rough proxy for relevance
            # Layer 1 results (closer match) would get higher relevance than Layer 2
            similarity = 0.7  # Default moderate similarity
            
            if "layer" in item:
                # Layer 1 is typically more relevant than Layer 2
                if item["layer"] == 1:
                    similarity = 0.8
                else:
                    similarity = 0.6
...
if __name__ == "__main__":
    ...
    result = assessment_system.process_query(
        user_query="Why are certain ethnic groups more prone to crime?",
